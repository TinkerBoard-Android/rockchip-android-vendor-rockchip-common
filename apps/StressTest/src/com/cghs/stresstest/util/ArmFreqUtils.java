...
public class ArmFreqUtils {
	...
	public static void setGovernorMode(String mode)
        ...
        public static void setGovernorMode_bigCore(String mode)
                        throws FileNotFoundException, IOException {
                // if(governor_freqFile.exists()){
                // try {
                PMwriteFile(governor_freqFile_bigCore, mode);
                // } catch (Exception re) {
                // re.printStackTrace();
                // Log.e(TAG, "IO Exception");
                // }
	...
	public static void setSpeedFreq(int value) throws FileNotFoundException,
        ...
        public static void setSpeedFreq_bigCore(int value) throws FileNotFoundException,
                        IOException {
                // if(setspeed_freqFile.exists()){
                // try {
                PMwriteFile(setspeed_freqFile_bigCore, String.valueOf(value));
                // } catch (Exception re) {
                // re.printStackTrace();
                // Log.e(TAG, "IO Exception");
                // }
	...
	public static List<String> getAvailableFrequencies() {
       ...
       public static List<String> getAvailableFrequencies_bigCore() {
                ...
                if (frequenciesFile_bigCore.exists()) {
                        try {
                                FileReader fread = new FileReader(frequenciesFile_bigCore);
                                BufferedReader buffer = new BufferedReader(fread);
                                String str = null;
                                StringBuilder sb = new StringBuilder();
                                while ((str = buffer.readLine()) != null) {
	...
	public static Integer getCurFrequencies() {
       ...
       public static Integer getCurFrequencies_bigCore() {
                Integer result = 0;
                if (cur_freqFile_bigCore.exists()) {
                        try {
                                FileReader fread = new FileReader(cur_freqFile_bigCore);
                                BufferedReader buffer = new BufferedReader(fread);
                                String str = null;
                                StringBuilder sb = new StringBuilder();
                                while ((str = buffer.readLine()) != null) {
