...
func init() {
  //该打印会在执行mm命令时，打印在屏幕上
  fmt.Println("libjpeghwenc want to conditional Compile")
  android.RegisterModuleType("cc_libjpeghwenc_prebuilt_library_shared", libjpeghwenc_LibraryFactory)
}

func libjpeghwenc_LibraryFactory() (android.Module) {
...
func globalArm64Defaults(ctx android.LoadHookContext) (Ex_multilibType) {
 ...
 if (strings.EqualFold(ctx.AConfig().Getenv("TARGET_BOARD_PLATFORM"),"rk3326")) {
        multilib.Lib32.Srcs = append(srcs,"arm/mpp_dev/libjpeghwenc.so")
        multilib.Lib64.Srcs = append(srcs,"arm64/mpp_dev/libjpeghwenc.so")
        ...
        multilib.Lib32.Srcs = append(srcs,"arm/libjpeghwenc.so")
        multilib.Lib64.Srcs = append(srcs,"arm64/libjpeghwenc.so")
 } else if (strings.EqualFold(ctx.AConfig().Getenv("TARGET_BOARD_PLATFORM"),"rk3399")) {
        multilib.Lib32.Srcs = append(srcs,"arm/mpp_dev/libjpeghwenc.so")
        multilib.Lib64.Srcs = append(srcs,"arm64/mpp_dev/libjpeghwenc.so")
 } else {
